...
export default function AdminRisk() {
  ...
  const { data: riskLimits, refetch } = trpc.risk.getRiskLimits.useQuery(undefined, {
    refetchInterval: 5000,
  });

  const updateLimitMutation = trpc.risk.updateRiskLimit.useMutation({
    onSuccess: () => {
      refetch();
  ...
  const handleUpdateLimit = (limitType: string, value: number) => {
    updateLimitMutation.mutate({ limitType, value });
  };
