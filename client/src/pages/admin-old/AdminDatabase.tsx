...
export default function AdminDatabase() {
  ...
  const [redisKeyspaceData, setRedisKeyspaceData] = useState<any>(null);
  const [postgresInfo, setPostgresInfo] = useState<any>(null);
  const [postgresTables, setPostgresTables] = useState<any>(null);
  const [parquetOverview, setParquetOverview] = useState<any>(null);
  const [parquetDirectories, setParquetDirectories] = useState<any>(null);
  ...
  const fetchData = async () => {
    try {
      setIsLoading(true);
      const [statsRes, redisInfoRes, redisKeyRes, pgInfoRes, pgTablesRes, parquetOverRes, parquetDirRes] = await Promise.all([
        fetch('/api/trpc/admin.getDatabaseStats').then(r => r.json()),
        fetch('/api/trpc/admin.getRedisInfo').then(r => r.json()),
        fetch('/api/trpc/admin.getRedisKeyspaceStats').then(r => r.json()),
        fetch('/api/trpc/admin.getPostgresInfo').then(r => r.json()),
        fetch('/api/trpc/admin.getPostgresTables').then(r => r.json()),
        fetch('/api/trpc/admin.getParquetOverview').then(r => r.json()),
        fetch('/api/trpc/admin.listParquetDirectories').then(r => r.json()),
      ]);
      
      setDbStats(statsRes.result?.data?.json || null);
      setRedisInfo(redisInfoRes.result?.data?.json || null);
      setRedisKeyspaceData(redisKeyRes.result?.data?.json || null);
      setPostgresInfo(pgInfoRes.result?.data?.json || null);
      setPostgresTables(pgTablesRes.result?.data?.json || null);
      setParquetOverview(parquetOverRes.result?.data?.json || null);
      setParquetDirectories(parquetDirRes.result?.data?.json || null);
    } catch (err) {
  ...
  const flushRedisCache = async () => {
    try {
      await fetch('/api/trpc/admin.flushRedisCache', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({})
      });
      await fetchData(); // Refresh after flush
    } catch (err) {
