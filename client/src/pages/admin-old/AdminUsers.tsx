...
export default function AdminUsers() {
  ...
  const fetchUsers = async () => {
    try {
      setLoading(true);
      const res = await fetch('/api/trpc/admin.getAllUsers').then(r => r.json());
      setUsers(res.result?.data?.json || null);
    } catch (err) {
  ...
  const handleCreateUser = async () => {
    try {
      await fetch('/api/trpc/admin.createUser', {
        method: 'POST',
  ...
  const handleDeleteUser = async (userId: string) => {
    if (confirm("Are you sure you want to delete this user?")) {
      try {
        await fetch('/api/trpc/admin.deleteUser', {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
